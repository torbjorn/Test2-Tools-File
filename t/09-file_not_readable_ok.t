...
SKIP: {
    ...
    my $events = intercept {
        file_not_readable_ok $handy_file; ## FAIL
        chmod("a-r", $handy_file) or bail_out("Failed removing read rights for tesing");
        file_not_readable_ok $handy_file; ## PASS
        unlink $handy_file if -e $handy_file;
        bail_out("A file that should not exist still exists.") if -e $handy_file;
        ## A nonexisting file is simply not not_readable
        file_not_readable_ok $handy_file; ## PASS
    };

    like $events, t2_events(qw(Fail Pass Pass)), "file_not_readable_ok: events";
